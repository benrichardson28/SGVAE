...
def load_datasets(config):
    ds_train = full_classification(train=True,dataset=config.dataset,action_select=config.action_select)
    ds_val = full_classification(train=True,dataset=config.dataset,action_select=config.action_select)
    train_indices, val_indices = split_indices(ds_train,4,config.dataset)
    ...
    ds_test = full_classification(train=False,dataset=config.dataset,action_select=config.action_select)
    ds_test.set_transform(ds_train.get_transform())

    return ds_train,ds_val,ds_test

def process_batch(config,data,labels,network,optimizer,loss_func,training=True):
    optimizer.zero_grad()
    #pdb.set_trace()
    ...
    #classifier_accuracy = (classifier_pred.data == labels).sum().item() / config.batch_size
    return classification_error, classifier_pred
    #return classification_error, classifier_accuracy, classifier_pred

def training_loop(config,train_loader,val_loader,network,optimizer,loss_func,writer):
    best_train_loss = 100
    #best_train_acc = 0
    best_val_loss = 100
    for ep in range(config.end_epoch):
        train_accuracy,train_loss = 0,0
        for data,actions,objects,labels in train_loader:
            #cle,cla,_ = process_batch(config,data,labels,network,optimizer,loss_func)
            cle,_ = process_batch(config,data,labels,network,optimizer,loss_func)
        ...
        val_loss,_,_,_,_ = eval_classifier(config,val_loader,network,optimizer,loss_func)
...
def eval_classifier(config,loader,network,optimizer,loss_func):
    with torch.no_grad():
        loss = 0
        all_predictions=[]
        all_labels=[]
        ...
        for data,actions,objects,labels in loader:
            cl,pred = process_batch(config,data,labels,network,
...
def run_all(config):
    ...
    val_loader = DataLoader(val,batch_size=config.batch_size,shuffle=True,**kwargs)
    test_loader = DataLoader(test,batch_size=config.batch_size,**kwargs)

    if type(config.test_properties) is not list:
        config.test_properties = [config.test_properties]
    for prop in config.test_properties:
        log_dir = 'baseline_classification_results'
        if config.save_folder is not None:
            if config.action_select is not None:
                log_dir = os.path.join(log_dir,f'{int(config.save_folder)}_{config.action_select}')
            else:
                log_dir = os.path.join(log_dir,f'{int(config.save_folder)}')
        log_dir = os.path.join(log_dir,f'{prop}_{int(config.save_dir_id)}')
        ...
        net = class_net(num_classes = class_cnt,
                        ...
                        paddings = config.paddings).to(config.device)
        optimizer = optim.Adam(
            list(net.parameters()),
            lr=config.initial_learning_rate,
            betas=(config.beta_1, config.beta_2),
            weight_decay=1)
        tr_loss,val_loss,best_model = training_loop(config,train_loader,val_loader,net,optimizer,loss_func,writer)
        ts_loss,preds,lbls,acts,objs = eval_classifier(config,test_loader,best_model,optimizer,loss_func)
